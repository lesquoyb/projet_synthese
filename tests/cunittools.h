...
//#define expected(error,func)        try{ auto test = [&]()func;test();return false;}catch(error e){return true;}
#define CUNIT(unitName) class unitName : public CUNIT {  public: unitName(){ CUNIT::_name = #unitName;
#define ENDUNIT run();}};
#define TESTCASE(name,content)   function<bool()> name = [&]() content ;\
